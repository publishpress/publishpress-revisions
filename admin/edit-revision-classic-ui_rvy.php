	...
	function actSubmitboxActions($post) {
		...
		if ($post_type_object && !current_user_can($post_type_object->cap->publish_posts)) : // Contributors don't get to choose the date of publish.
			/* translators: Publish box date string. 1: Date, 2: Time. See https://www.php.net/manual/datetime.format.php */
				$date_string = esc_html__( '%1$s at %2$s', 'revisionary' );
			/* translators: Publish box date format, see https://www.php.net/manual/datetime.format.php */
				$date_format = esc_html_x('M j, Y', 'publish box date format', 'revisionary' );
			/* translators: Publish box time format, see https://www.php.net/manual/datetime.format.php */
				$time_format = esc_html_x('H:i', 'publish box time format', 'revisionary' );

			if ( 0 !== $post->ID ) {
				if ( 'future' === $post->post_status ) { // Scheduled for publishing at a future date.
					/* translators: Post date information. %s: Date on which the post is currently scheduled to be published. */
					$stamp = esc_html__( 'Scheduled for: %s', 'revisionary' );
				} elseif ( 'publish' === $post->post_status || 'private' === $post->post_status ) { // Already published.
					/* translators: Post date information. %s: Date on which the post was published. */
					$stamp = esc_html__( 'Published on: %s', 'revisionary' );
				} elseif ( '0000-00-00 00:00:00' === $post->post_date_gmt ) { // Draft, 1 or more saves, no date specified.
					$stamp = __( 'Publish <b>immediately</b>', 'revisionary' );
				} elseif ( time() < strtotime( $post->post_date_gmt . ' +0000' ) ) { // Draft, 1 or more saves, future date specified.
					/* translators: Post date information. %s: Date on which the post is to be published. */
					$stamp = esc_html__( 'Schedule for: %s', 'revisionary' );
				} else { // Draft, 1 or more saves, date specified.
					/* translators: Post date information. %s: Date on which the post is to be published. */
					$stamp = esc_html__( 'Publish on: %s', 'revisionary' );
				}
				$date = sprintf(
					$date_string,
			...
			<div class="misc-pub-section curtime misc-pub-curtime">
				...
				<a href="#edit_timestamp" class="edit-timestamp hide-if-no-js" role="button">
					<span aria-hidden="true"><?php esc_html_e( 'Edit', 'revisionary' ); ?></span>
					<span class="screen-reader-text">
						<?php
						/* translators: Hidden accessibility text. */
						esc_html_e( 'Edit date and time', 'revisionary' );
						?>
					</span>
				</a>
				<fieldset id="timestampdiv" class="hide-if-js">
					<legend class="screen-reader-text">
						<?php
						/* translators: Hidden accessibility text. */
						esc_html_e( 'Date and time', 'revisionary' );
						?>
					</legend>
