...
ensureCoreBlocksRegistered();const currentRaw=grammarParse(currentContent||'');const previousRaw=grammarParse(previousContent||'');const mergedRaw=diffRawBlocks(currentRaw,previousRaw);const parsedBlocks=mergedRaw.map((rawBlock)=>{if(!rawBlock.blockName&&!(rawBlock.innerHTML||'').trim()){return null;}
...
const scopedCss=scopeEditorCss(style.css,'.visual-post-compare-revision__live-preview');return scopedCss?el('style',{key:'vpc-editor-style-'+index},scopedCss):null;}).filter(Boolean);const presentation=comparison.presentation||{};const linkedValue=(post,value)=>el('a',{href:post.url||'#',className:'visual-post-compare-revision__date-link'},value);const statusCaption=(post)=>presentation.mimeTypeStatus?post.mimeTypeStatusLabel:post.statusLabel;const dateLine=(post,prefix,value,className)=>el('div',{className:'visual-post-compare-revision__date-row '+className},el('span',{className:'visual-post-compare-revision__date-prefix'},prefix),linkedValue(post,value));const authorLine=(post)=>post.authorName?el('div',{className:'visual-post-compare-revision__author'},sprintf(__('by %s','revisionary'),post.authorName)):null;const details=(post)=>[presentation.showModified!==false?dateLine(post,presentation.modifiedPrefix||__('Modified: ','revisionary'),post.modifiedLabel||post.modified,'is-modified-date'):null,presentation.showPostDate?dateLine(post,presentation.postDatePrefix||__('Post Date: ','revisionary'),post.postDateLabel||post.postDate,'is-post-date'):null,presentation.showAuthor!==false?authorLine(post):null,].filter(Boolean);const isCurrentSelected=Number(comparison.to.id)===Number(comparison.from.id);const currentMeta=el('div',{key:'current'},el('span',{className:'visual-post-compare-revision__status'},__('Current','revisionary')),el('strong',null,comparison.from.title||sprintf(__('Post %d','revisionary'),comparison.from.id)),...details(comparison.from));const revisionMeta=isCurrentSelected?el('div',{},''):el('div',{key:'revision'},presentation.showRightStatus!==false?el('span',{className:'visual-post-compare-revision__status'},statusCaption(comparison.to)||__('Revision','revisionary')):null,el('strong',null,comparison.to.title||sprintf(__('Post %d','revisionary'),comparison.to.id)),...details(comparison.to),comparison.canApprove?el(Button,{variant:'primary',className:'visual-post-compare-revision__approve',onClick:onApprove,isBusy:isApproving,disabled:isApproving,},isApproving?__('Approving…','revisionary'):config.approveCaption||__('Approve','revisionary')):null);const metaColumns=config.currentPostFirst===false?[revisionMeta,currentMeta]:[currentMeta,revisionMeta];const diffMarkerNav=hasCanvasOverflow&&diffMarkers.length?el('nav',{className:'visual-post-compare-revision__diff-markers','aria-label':__('Document changes','visual-post-compare'),},...diffMarkers.map((marker)=>el('button',{key:marker.key,type:'button',className:'visual-post-compare-revision__diff-marker is-'+marker.status,title:markerLabel(marker.status),'aria-label':markerLabel(marker.status),style:{top:marker.top+'%','--vpc-marker-height':marker.height+'%'},onClick:()=>{const canvas=canvasRef.current;if(!canvas||!marker.node){return;}
const canvasRect=canvas.getBoundingClientRect();const nodeRect=marker.node.getBoundingClientRect();const nodeTop=canvas.scrollTop+nodeRect.top-canvasRect.top;const targetTop=nodeTop-Math.max(0,(canvas.clientHeight-nodeRect.height)/2);canvas.scrollTo({top:Math.max(0,targetTop),behavior:'smooth'});},}))):null;return el('section',{className:'visual-post-compare-revision'},el('div',{className:'visual-post-compare-revision__legend','aria-label':__('Comparison legend','visual-post-compare')},el('span',{className:'is-added'},__('Added','visual-post-compare')),el('span',{className:'is-removed'},__('Removed','visual-post-compare')),el('span',{className:'is-modified'},__('Modified','visual-post-compare'))),el('div',{className:'visual-post-compare-revision__body'},diffResult.error?el(Notice,{status:'error',isDismissible:false},diffResult.error.message):el('div',{className:'visual-post-compare-revision__canvas-shell'},el('div',{className:'visual-post-compare-revision__canvas',ref:canvasRef},...editorStyleElements,el('style',null,REVISION_DIFF_STYLES),el('div',{className:'visual-post-compare-revision__svg-filters','aria-hidden':true,dangerouslySetInnerHTML:{__html:REVISION_REMOVED_FILTER_SVG},}),el('div',previewProps)),diffMarkerNav),el('aside',{className:'visual-post-compare-revision__sidebar'},metaColumns[0],metaColumns[1])));}
function normalizeCurrentPostPlacement(comparison){if(!comparison||!Array.isArray(comparison.posts)||!comparison.from){return comparison;}
const fromId=Number(comparison.from.id);const currentPost=comparison.posts.find((post)=>Number(post.id)===fromId);if(!currentPost){return comparison;}
const otherPosts=comparison.posts.filter((post)=>Number(post.id)!==fromId);const posts=config.currentPostFirst===false?[...otherPosts,currentPost]:[currentPost,...otherPosts];return{...comparison,posts};}
function App(){const[comparison,setComparison]=useState(null);const[error,setError]=useState(null);const[isApproving,setIsApproving]=useState(false);const[approvalNotice,setApprovalNotice]=useState(null);useEffect(()=>{apiFetch({path:config.restPath+'?from='+encodeURIComponent(config.from)+'&to='+encodeURIComponent(config.to)+(config.comparisonKey?'&comparison='+encodeURIComponent(config.comparisonKey):''),}).then((loadedComparison)=>setComparison(normalizeCurrentPostPlacement(loadedComparison))).catch(setError);},[]);if(error){return el(Notice,{status:'error',isDismissible:false},(error&&error.message)||__('Unable to load this comparison.','revisionary'));}
if(!comparison){return el('div',{className:'visual-post-compare-loading'},el(Spinner),__('Loading comparison…','revisionary'));}
const approve=async()=>{if(isApproving||!comparison.canApprove||Number(comparison.to.id)===Number(comparison.from.id)){return;}
setIsApproving(true);setApprovalNotice(null);try{const updatedComparison=await apiFetch({path:config.approveRestPath+'?from='+encodeURIComponent(config.from)+'&to='+encodeURIComponent(comparison.to.id)+(config.comparisonKey?'&comparison='+encodeURIComponent(config.comparisonKey):''),method:'POST',});setComparison(normalizeCurrentPostPlacement(updatedComparison));setApprovalNotice({status:'success',message:__('The target post content has been replaced with the approved content.','revisionary')});}catch(approveError){setApprovalNotice({status:'error',message:(approveError&&approveError.message)||__('Unable to approve this comparison.','revisionary'),});}finally{setIsApproving(false);}};return el('div',{className:'visual-post-compare-screen'},el('header',{className:'visual-post-compare-screen__header'},el('h1',null,config.headline||__('Compare Revisions','revisionary'))),approvalNotice?el(Notice,{status:approvalNotice.status,isDismissible:true,onRemove:()=>setApprovalNotice(null)},approvalNotice.message):null,el(ComparisonSlider,{comparison,onSelect:(post)=>{setApprovalNotice(null);setComparison({...comparison,to:post,selectedId:Number(post.id)});},}),el(RevisionPreview,{comparison,onApprove:approve,isApproving}));}
